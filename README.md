...
| **Git 仓库管理** | 在左侧面板查看改动、暂存文件、提交和 Git 版本历史 | 开 |
...
Git 面板位于左侧导航栏，按 Workspace 保存状态并跨会话复用。未初始化的目录可直接初始化仓库；已初始化的仓库支持查看暂存文件和未提交文件、暂存/取消暂存、丢弃更改、填写提交说明、切换分支和浏览提交历史。模块关闭后会移除左侧标签及对应 Host Git RPC，不影响其他模块。
